...
SDLWindow::SDLWindow(int width, int height, std::string_view title) : w(width), h(height), title(title) {
	...
	rc = SDL_CreateWindowAndRenderer(title.data(), width, height, 0, &window, &renderer);
	if (rc != 0) {
		SDL_Quit();
...
SDLWindow::~SDLWindow() {
	SDL_DestroySurface(images.charset);
	SDL_DestroySurface(images.pieces);
	SDL_DestroySurface(images.piecesMin);
...
SDL_Surface* SDLWindow::SDL_CreateRGBSurface(Uint32 flags, int width, int height, int depth, Uint32 Rmask, Uint32 Gmask, Uint32 Bmask, Uint32 Amask)
{
